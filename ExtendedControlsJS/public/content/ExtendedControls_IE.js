...
var TableUpdateDataParams = function TableUpdateDataParams(changedRows, newRows, deletedRows, allRows) {
    _classCallCheck(this, TableUpdateDataParams);
...
var EventParams = function EventParams(eventName, handler) {
...
(function ($) {
    $.fn.DataTable = function (options) {
        ...
        control.Events = {
            NewEventHandler: function NewEventHandler(event, handler) {
        ...
        control.Data = {
            SetTableData: function SetTableData(value) {
                control.tableData = value;
                control.updateTable();
                return "";
            },
            SortTableData: function SortTableData(column, desc) {
        ...
        control.generateTable = function () {
            ...
            control.sortedData.forEach(function (row, i) {
                ...
                control.columns.forEach(function (col, i) {
                    ...
                    if (showField) {
                        var dataValueType = _typeof(row[col]);
    ...
    $.fn.ExtendedMap = function (options) {
        ...
        extendedMap.initMap = function () {
            ...
            $(areas).each(function (i, e) {
                ...
                switch (areaShape) {
                    ...
                    case "rect":
                        ...
                        var w = x2 - x;
                        var h = y2 - y;
                        var shapeHTML = '<rect id="$id$" class="shape" areashape="$areashape$" x="$x$" y="$y$" width="$w$" height="$h$" href="$href$" params=$params$><title>$title$</title></rect>'.replace("$x$", x).replace("$y$", y).replace("$w$", w).replace("$h$", h);
                        break;
                    ...
                    case "circle":
                        ...
                        var shapeHTML = '<circle id="$id$" class="shape" areashape="$areashape$" cx="$x$" cy="$y$" r="$r$" href="$href$" params=$params$><title>$title$</title></circle>'.replace("$x$", x).replace("$y$", y).replace("$r$", r);
                        break;
                }
                shapeHTML = shapeHTML.replace("$title$", shapeTitle).replace("$href$", shapeHREF).replace("$areashape$", areaShape).replace("$params$", " '" + areaParams + "'").replace("$id$", extendedMapID + "_svg_shape" + shapeCount);
            ...
            $(shapeControls).each(function (i, e) {
                ...
                infoTextHTML = infoTextHTML.replace("$left$", left + "px").replace("$top$", top + "px").replace("$id$", shapeID + "_info_panel").replace("$params$", shapeParams);
...
(function ($) {
    $.fn.buttonGroup = function (options) {
        ...
        control.updateBoundValue = function (newValue) {
            var objectType = _typeof(control.boundValueObject());
